...
function configHooks {
  ...
  envsubst < "${WORK_DIR}"/debian-live/config/hooks/normal/9000-install-flathub-flatpak-packages-system.hook.chroot.template > "${BUILD_DIR}"/config/hooks/normal/9000-install-flathub-flatpak-packages-system.hook.chroot
  if [ "$?" -ne 0 ]; then
...
function installPrerequisites {
  ...
  sudo apt install --no-install-recommends --yes /tmp/live-build_"${DEBIAN_LIVE_BUILD_VERSION}"_all.deb debian-archive-keyring perl
  if [ "$?" -ne 0 ]; then
