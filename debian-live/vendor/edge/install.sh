...
if [[ "$DEBIAN_ARCH" == "amd64" ]]; then
    ...
    curl --continue-at - --silent --location https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-stable/microsoft-edge-stable_131.0.2903.99-1_amd64.deb --output "${BUILD_DIR}"/config/packages.chroot/microsoft-edge-stable_131.0.2903.99-1_amd64.deb
    if [ "$?" -ne 0 ]; then
