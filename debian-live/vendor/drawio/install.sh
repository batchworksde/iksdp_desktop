...
if [ "${DEBIAN_ARCH}" = "amd64" ]; then
    ...
    curl --continue-at - --silent --location url -s https://api.github.com/repos/jgraph/drawio-desktop/releases/latest | grep browser_download_url | grep '\.deb' | cut -d '"' -f 4 | grep -i ${DEBIAN_ARCH} | wget --no-verbose --output-document="${BUILD_DIR}"/config/packages.chroot/drawio-amd64-26.0.4.deb -i - 
