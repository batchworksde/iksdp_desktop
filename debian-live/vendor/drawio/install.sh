...
if [ "${DEBIAN_ARCH}" = "amd64" ]; then
    ...
    curl --continue-at - --silent https://api.github.com/repos/jgraph/drawio-desktop/releases/latest | yq --input-format json ".assets[] | select(.name == \"drawio-amd64-*.deb\") | .browser_download_url" | wget --no-verbose --output-document=./drawio_amd64.deb --input-file -
